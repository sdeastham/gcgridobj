...
G5_AP = np.array([ 0.000000e+00, 4.804826e-02, 6.593752e+00, 1.313480e+01,
...
G5_BP = np.array([ 1.000000e+00, 9.849520e-01, 9.634060e-01, 9.418650e-01,
...
G5_grid = vert_grid(G5_AP, G5_BP)

# Reduced grid
G5_AP_47L = np.zeros(48)
G5_BP_47L = np.zeros(48)
...
for i_lev in range(1,37):
    ...
    G5_AP_47L[i_lev] = G5_AP[i_lev]
    G5_BP_47L[i_lev] = G5_BP[i_lev]
...
for lump_seg in range(2):
    ...
    for i_lev_offset in range(number_lumped[lump_seg]):
        ...
        G5_AP_47L[i_lev] = G5_AP[i_lev_72]
        G5_BP_47L[i_lev] = G5_BP[i_lev_72]
        ...
        dp_total = G5_AP_47L[i_lev-1] - G5_AP_47L[i_lev]
        ...
        for i_lev_offset_72 in range(skip_size):
            ...
            xmat_s[xct] = (G5_AP[x_lev_72] - G5_AP[x_lev_72+1])/dp_total
    start_pt = i_lev

# Do last entry separately (no layer to go with it)
xmat_72to47 = scipy.sparse.coo_matrix((xmat_s,(xmat_i,xmat_j)),shape=(72,47))

G547_grid = vert_grid(G5_AP_47L, G5_BP_47L)

CAM_AP = np.flip(np.array([ 219.4067,   489.5209,   988.2418,   1805.201,        
...
CAM_BP = np.flip(np.array([ 0.,         0.,         0.,         0.,             
                            0.,         0.,         0.,         0.,             
...
CAM26_grid = vert_grid(CAM_AP, CAM_BP)
